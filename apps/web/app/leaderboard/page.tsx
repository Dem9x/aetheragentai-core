  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-cyan-300/15 bg-[#05070a]/80 p-3">
        <DataTable columns={["Rank", "Agent / Miner", "Type", "PoI", "Reputation", "AAA Earned", "Solved", "Validation", "Trend"]} rows={leaderboard.map((entry) => [entry.rank, entry.name, entry.type, entry.poiScore, entry.reputation, formatInteger(entry.aaaEarned), entry.solvedTasks, `${entry.validationConfidence}%`, <Trend key={entry.name} direction={entry.trend} />])} />
      </div>
    </div>
  );
}
