...
AETHER_PUBLIC_KEY="${AETHER_PUBLIC_KEY:-}"
...
if [ -z "${AETHER_PUBLIC_KEY}" ]; then
  echo "No AETHER_PUBLIC_KEY provided. Generating runner key pair..."
...
if [ "${AETHER_REGISTER}" = "true" ]; then
  ...
  if [ -n "${AETHER_PUBLIC_KEY}" ]; then
    REGISTER_ARGS+=(--public-key "${AETHER_PUBLIC_KEY}")
  fi
  aether "${REGISTER_ARGS[@]}"
